...
export default function TierCard({ tier, billing, index, onSelect }) {
  ...
  return (
    ...
    >
      {featured && (
        <span className="font-mono absolute -top-[9px] left-8 bg-ember px-3 py-[3px] text-[0.72rem] tracking-[0.14em] text-bone uppercase">
      ...
      <header>
        ...
        <p className="font-mono mt-1 text-[0.75rem] tracking-[0.14em] text-ash uppercase">
          {tier.subtitle}
        </p>
      </header>
      ...
      <div className="mt-6 flex items-baseline gap-2 border-t border-bone/8 pt-6">
        ...
        <span className="font-mono text-[0.78rem] tracking-[0.1em] text-ash">
          {BILLING[billing].suffix}
        </span>
      </div>

      <dl className="font-mono mt-5 grid grid-cols-2 gap-y-2 text-[0.76rem] tracking-[0.12em] uppercase">
        <dt className="text-ash">Сила</dt>
        <dd className="text-right text-brass">{tier.potency}</dd>
        <dt className="text-ash">Срок</dt>
      ...
      <ul className="mt-7 flex flex-1 flex-col gap-3 border-t border-bone/8 pt-6 text-[1.02rem] leading-snug">
        {tier.features.map((f) => (
          <li key={f} className="flex gap-3 text-bone">
            <Mark />
            {f}
          </li>
        ))}
        {tier.excluded.map((f) => (
          <li key={f} className="flex gap-3 text-ash line-through">
            <Mark struck />
            {f}
          </li>
        ))}
      ...
      <button
        ...
        className={`font-mono mt-8 cursor-pointer border px-6 py-3 text-[0.8rem] tracking-[0.12em] uppercase transition-all duration-300 ${
          featured
